...
const setupWatcher = (p) => {
  const watcher = chokidar.watch(p, { // data
    ignoreInitial: true,
...
const setupCouch = () => {
  ...
  let cnt = 0
  couchdbEvents.timer = setInterval(() => console.log('couchdbEvents:', ++cnt, Object.keys(couchdbEvents).join()), 6000)
  return couchdbEvents
}

const watcher = setupWatcher('data/**/*.json')
const coucher = setupCouch()
